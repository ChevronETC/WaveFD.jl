...
@inline function block_zero_timestep!(p::PureJulia2DAcoIsoDenQ_DEO2_FDTD, izrng, ixrng)
    for kx in ixrng
...
@inline function zero_timestep!(p::PureJulia2DAcoIsoDenQ_DEO2_FDTD)
    for iz in 5:p.nbz_cache:p.nz-4
        Threads.@threads for ix in 5:p.nbx_cache:p.nx-4
            block_zero_timestep!(p, iz:min(p.nz-4,iz+p.nbz_cache-1), ix:min(p.nx-4,ix+p.nbx_cache-1))
        end
    end
    nothing
end
