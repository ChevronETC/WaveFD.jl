...
function Prop2DAcoTTIDenQ_DEO2_FDTD(;
    ...
    p = ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_alloc, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD),
        Ptr{Cvoid},
        (Clong, Clong,    Clong, Clong, Clong,   Cfloat, Cfloat, Cfloat, Clong, Clong),
         fs,    nthreads, nx,    nz,    nsponge, dx,     dz,     dt,     nbx,   nbz)

    ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_SetupDtOmegaInvQ, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD),
        Cvoid,
...
free(prop::Prop2DAcoTTIDenQ_DEO2_FDTD) = ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_free, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD), Cvoid, (Ptr{Cvoid},), prop.p)
...
for _f in (:V, :Eps, :Eta, :SinTheta, :CosTheta, :B, :F, :PSpace, :MSpace, :PCur, :POld, :MCur, :MOld, :TmpPg1, :TmpPg3, :TmpMg1, :TmpMg3, :DtOmegaInvQ)
    ...
    @eval $(_f)(prop::Prop2DAcoTTIDenQ_DEO2_FDTD) = unsafe_wrap(Array, ccall(($symf, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD), Ptr{Float32}, (Ptr{Cvoid},), prop.p), size(prop), own=false)
end

propagateforward!(prop::Prop2DAcoTTIDenQ_DEO2_FDTD) = ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_TimeStep, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD), Cvoid, (Ptr{Cvoid},), prop.p)
...
scale_spatial_derivatives!(prop::Prop2DAcoTTIDenQ_DEO2_FDTD) =
    ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_ScaleSpatialDerivatives, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD), Cvoid, (Ptr{Cvoid},), prop.p)
...
function forwardBornInjection!(prop::Prop2DAcoTTIDenQ_DEO2_FDTD,::Type{Prop2DAcoTTIDenQ_DEO2_FDTD_Model_VEA},dmodel,wavefield)
    ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_ForwardBornInjection_VEA, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD), Cvoid,
...
function adjointBornAccumulation!(prop::Prop2DAcoTTIDenQ_DEO2_FDTD,::Type{Prop2DAcoTTIDenQ_DEO2_FDTD_Model_VEA},dmodel,wavefield)
    ccall((:Prop2DAcoTTIDenQ_DEO2_FDTD_AdjointBornAccumulation_VEA, Wave._jl_libprop2DAcoTTIDenQ_DEO2_FDTD), Cvoid,
        (Ptr{Cvoid}, Ptr{Cfloat}, Ptr{Cfloat}, Ptr{Cfloat}, Ptr{Cfloat},       Ptr{Cfloat},       Ptr{Cfloat},         Ptr{Cfloat}),
         prop.p,     dmodel["v"], dmodel["ϵ"], dmodel["η"], wavefield["pold"], wavefield["mold"], wavefield["pspace"], wavefield["mspace"])
end
